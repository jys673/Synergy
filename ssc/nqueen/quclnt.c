...
{ 
	...
	while (tp_cnt>0)
	{
		...
		cnf_tsread(tsd, tpname, iituple, 0);
		total=iituple[1];
		tp_cnt=iituple[0];
	}
	t1 = time((long *)0) - t0;
	printf("n=%d,P=%d",n,P);
	fd = fopen("nq_nxdr.time","a");
	/* Stat file format: Console TotalSolutions N P ElapsedTime */
        printf("(%s) %d Solutions for %d QUEENS. P(%d) in %d sec.\n",
			host,total,n,P,t1);
