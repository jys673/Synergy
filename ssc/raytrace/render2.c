/*
  This is the render program for SNGRAY2,
  I modify it from render.c.
...
/* There following codes had been change by C Fu to port
   the POV to parallel. */
...
/* ---------------  Add variables -------------*/
  ...
  test_chunk_size(job_tuple,output_tuple);
 
 /* need to solve the problem */

  sleep(15);  /* every worker will finish the test_chunk_size in 
                 different time, so we need to wait a little moment.
                 the sleep time is also depend on the speed of different
                 machine combinations, in yoda, with 7 hosts, this is 
                 a safe number! */

  RUN=0;
 printf("\n Worker begin to calc... ");
 fflush(stdout);
 RUN=1;
while(RUN)
{ 
