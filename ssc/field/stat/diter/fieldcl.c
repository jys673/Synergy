...
/* Open Tuple Spaces */
    ...
    if  ( (tsinp=cnf_open("input",0)) < 0) {
        printf("Field MASTER: Error opening input Tuple space \n");
        exit(0);
    }
    if  ( (tsout=cnf_open("output",0)) < 0) {
        printf("Field MASTER: Error opening output Tuple space \n");
        exit(0);
    }
...
((fp=fopen("field.cfg","r"))==NULL) {
    ...
    sprintf(filenm, "hps.field.%d.out", NRES);
    if  
((outfd=fopen(filenm,
                      "w"))==NULL) {
    ...
    sprintf(filenm, "hps.field.%d.stat", NRES);
    if  
((sfd=fopen(filenm,
                      "w"))==NULL) {
...
****/
    itmp = 15;
    for (niter = itmp; niter <= 20; niter=niter+5) {
...
/*---------------------------------------------------------------------------*/
	...
	cnf_dinit();  /* Initialize local tid list */
	cnf_spzap(tsinp);
    } while (!GaSeOnce());
...
InsertAsgn() 
{
    ...
    sprintf(tid, "asgn%d", blindex);
    tplength = sizeof(asgn);
    status = cnf_tsput(tsinp, tid, (char *)&asgn, tplength);

    /* Insert the current blstart for the processor having the LEFT partition */
    for  (i=0; i<XRES; i++) {
         row[i+1] = phiold[blstart][i];
    }
    tplength = (XRES+1) * sizeof(double);
    if (blindex > 0) sprintf(tid, "asgn%d", blindex-1);
    else strcpy(tid, "asgn0");
    sprintf(tpname, "row%d", blstart);
/*
    printf("**master dput l_border. tid(%s) tpname(%s)\n", tid, tpname);
*/
    status = cnf_dput(tsinp, tid, tpname, (char *)row, tplength);
   
    /* Insert the current blstop for the processor having the RIGHT partition*/ 
    for  (i=0; i<XRES; i++) {
         row[i+1] = phiold[blstop][i];
    }
    tplength = (XRES+1) * sizeof(double);
    if (blindex < NTPL-1) sprintf(tid, "asgn%d", blindex+1);
    else sprintf(tid, "asgn%d", NTPL-1);
    sprintf(tpname, "row%d", blstop);
    status = cnf_dput(tsinp, tid, tpname, (char *)row, tplength);
/*
printf("**master dput h_border: tid(%s) tpname(%s)\n", tid, tpname);
*/
