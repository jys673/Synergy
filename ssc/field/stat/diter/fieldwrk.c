...
void GetAssign()
{
    ...
    cnf_dinit();  /* clean up the tid list due to new assignment */
printf(" ***** Worker got (%s)\n", tpname);
...
void GetBorder()
{
    ...
    status = cnf_dread(tpname, (char *)row, 0);
    for  (i=0; i<XRES; i++) {
...
void OutputBlock()
{
    ...
    tplength = sizeof(asgn);
    cnf_dzap();  /* remove all dps tuples */
    PackBlock();
...
void SubmitBorder()
{
    ...
    /* tid=asgnxx, row%d assignments stay the same as before */
    row[0] = tplog.ExtIter;
    for  (i=0; i<XRES; i++) {
         row[i+1] = phioldsm[1][i];
    }
    ...
    tplength = (XRES+1) * sizeof(double);
    tidx = atoi(&tid[4]);
    if (tidx == 0) strcpy(ntid,"local"); /* send a copy to self */
    else sprintf(ntid,"asgn%d", tidx-1);  /* send to the left tid */
/*
printf(" Lworker(%d) sending (%s) to tid(%s)\n", tidx, tpname, ntid);
*/
    status = cnf_dput(tsinp, ntid, tpname, (char *)row, tplength);
    ...
    sprintf(tpname, "row%d", blstop);
    tplength = (XRES+1) * sizeof(double);
    if (tidx == NTPL-1) strcpy(ntid,"local"); /* need to send a copy self */
    else sprintf(ntid,"asgn%d", tidx+1);  /* send to the right tid */
/*
printf(" Rworker(%d) sending (%s) to tid(%s)\n", tidx, tpname, ntid);
*/
    status = cnf_dput(tsinp, ntid, tpname, (char *)row, tplength);
}
