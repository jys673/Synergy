...
while (1)
{
    ...
    _tp_size = sizeof(int);
    _tp_i_234567 = &i;
    _tp_size = _read_data(_distributor, _tp_name, (char *)_tp_i_234567, _tp_size);
    if (_tp_size < 0) exit(-1);
    ...
    sprintf(_tp_name, "float(%d)(%d):outMat#%s[%d~%d,%d][%d~%d,%d]@%d", (N), (N), "234567", (i), (i+M), 1, (j+1), (N), 1, sizeof(float));
    ...
    if (_tp_size < 0) exit(-1);
    for (_x0_234567 = (i), _y0_234567 =0; _x0_234567 < (i+M); _x0_234567 +=1, _y0_234567 ++) {
    ...
    /* <read var="outMat" type="float[N(k1)   ][N(i~i+M)]"/> */
    sprintf(_tp_name, "float(%d)(%d):outMat#%s[%d~%d,%d][%d~%d,%d]@%d", (N), (N), "234567", _k1_start, _k1_stop, _k1_step, (i), (i+M), 1, sizeof(float));
    ...
    for (_x0_234567 = _k1_start, _y0_234567 =0; _x0_234567 < _k1_stop; _x0_234567 +=_k1_step, _y0_234567 ++) {
        for (_x1_234567 = (i), _y1_234567 =0; _x1_234567 < (i+M); _x1_234567 +=1, _y1_234567 ++) {

            outMat[_x0_234567][_x1_234567] = _tp_outMat_234567[_y0_234567 * ((i+M) - (i)) + _y1_234567];
        }
    }
    ...
    /* <read var="outMat" type="float[N(k1)   ][N(j+1~N)]"/> */
    sprintf(_tp_name, "float(%d)(%d):outMat#%s[%d~%d,%d][%d~%d,%d]@%d", (N), (N), "234567", _k1_start, _k1_stop, _k1_step, (j+1), (N), 1, sizeof(float));
    _tp_size = (((_k1_stop - _k1_start - 1) / _k1_step + 1) * ((N) - (j+1))) * sizeof(float);
    _tp_outMat_234567 = (float *)malloc(_tp_size);
    _tp_size = _read_data(_distributor, _tp_name, (char *)_tp_outMat_234567, _tp_size);
    if (_tp_size < 0) exit(-1);
    for (_x0_234567 = _k1_start, _y0_234567 =0; _x0_234567 < _k1_stop; _x0_234567 +=_k1_step, _y0_234567 ++) {
...
/* </target> */
    ...
    /* <send var="outMat" type="float[N(k1)   ][N(j+1~N)]"/> */
    sprintf(_tp_name, "float(%d)(%d):outMat#%s[%d~%d,%d][%d~%d,%d]@%d", (N), (N), "234567", _k1_start, _k1_stop, _k1_step, (j+1), (N), 1, sizeof(float));
    _tp_size = (((_k1_stop - _k1_start - 1) / _k1_step + 1) * ((N) - (j+1))) * sizeof(float);
    _tp_outMat_234567 = (float *)malloc(_tp_size);
    for (_x0_234567 = _k1_start, _y0_234567 =0; _x0_234567 < _k1_stop; _x0_234567 +=_k1_step, _y0_234567 ++) {
        for (_x1_234567 = (j+1), _y1_234567 =0; _x1_234567 < (N); _x1_234567 +=1, _y1_234567 ++) {
            ...
            _tp_outMat_234567[_y0_234567 * ((N) - (j+1)) + _y1_234567] = outMat[_x0_234567][_x1_234567];
        }
    }
