...
int seedArray[] = {111, 11, 31, 121, 313, 757, 0};
...
{
	return (int) (-100000 * ((e1->pbyw) - (e2->pbyw))) ;
}
...
double bfsKnap(ELEM *PbyW, int M, int N) 
{
...
main()
{
	...
	double capacity;
	double elapsedTime;
	double optimalFilling;
/*
	printf("Number of elements ");
	scanf("%d", &num);
*/

    for(seedIndex = 0; seedArray[seedIndex] != 0; ++seedIndex){
	SEED = seedArray[seedIndex];
	srand(SEED);
	num = 22;
	fp = fopen("bfs-hcr.seq","a");
...
*/
	optimalFilling = bfsKnap(items, capacity, num);
	elapsedTime = endTime;
	printf("Time taken %g\n", elapsedTime);
	...
	if(elapsedTime == 0)
		elapsedTime = 1;
	fprintf(fp, "#Elements = %d, Capacity %g, OptimalFilling %g, Elapsed Time(in seconds) %d, #Operations %g,\
		MFLOPS %g SEED %d\n", num, capacity, optimalFilling, elapsedTime, numOps, (numOps/elapsedTime)/1000000.0, SEED);
	fclose(fp);
	sleep(10);
   } /*end seed loop */
}	
